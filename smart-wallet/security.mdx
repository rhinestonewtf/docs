...
<Tip>If you're starting from scratch, consider [JWT authentication](/intents/configuration/jwt-authentication) as a structured alternative to the proxy pattern below. It keeps the signing key server-side, issues short-lived access tokens to the client, and supports per-intent sponsorship policies without hand-rolling a proxy.</Tip>
