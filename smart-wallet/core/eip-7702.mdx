...
<Info>
The init signature is unique to the account address and only needs to be signed once. Cache it after the first request and pass it for each transaction on a new chain where the account is not yet deployed. Use `account.isDeployed` to check whether the account is already deployed on a given chain and skip passing the signature when it is.
</Info>
