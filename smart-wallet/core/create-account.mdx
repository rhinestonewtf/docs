...
The SDK derives a deterministic account address from the owner configuration. The same owners always produce the same address — no deployment transaction required upfront.

## Custom nonce

By default, the SDK uses a nonce of `0` to derive the account address. Pass a custom nonce to get a different address for the same set of owners — useful when you need multiple accounts per signer.
