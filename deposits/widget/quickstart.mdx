...
```bash
npm install @rhinestone/deposit-modal viem wagmi @tanstack/react-query @reown/appkit @reown/appkit-adapter-wagmi
```
...
The `data` object includes the source `txHash`, the `destinationTxHash` on the target chain, the `amount`, and chain/token details. See [status tracking](/deposits/widget/status-tracking) for all available callbacks.
