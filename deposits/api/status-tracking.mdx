...
- **Retries** — failed deliveries are retried multiple times before the event is marked `failed`. Events that exhaust their retries can still be replayed on demand via the `POST /webhooks/events/{id}/resend` endpoint, which reuses the original `eventId`.
