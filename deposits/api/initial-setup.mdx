...
Control who pays for gas, bridging, and swap fees on a per-chain basis. Chains are identified by [CAIP-2](https://chainagnostic.org/CAIPs/caip-2) identifiers (e.g. `eip155:8453` for Base).

| Option | Values | Description |
|---|---|---|
| `gas` | `"all"`, `"deployed"`, `"none"` | `"all"` sponsors gas for all accounts. `"deployed"` only sponsors already-deployed accounts (skips first-time deployment gas). Default: `"none"`. |
| `bridging` | `"all"`, `"none"` | `"all"` covers the relayer bridging fee. Default: `"none"`. |
| `swap` | `"all"`, `"none"` | `"all"` covers swap fees when a token conversion is needed. Default: `"none"`. |
...
<Note>
When a fee isn't sponsored, it's deducted from the bridged amount. The user receives slightly less than they deposited.
</Note>
