...
await fetch(`${DEPOSIT_SERVICE_URL}/setup`, {
  ...
  body: JSON.stringify({
    params: {
      sponsorship: {
        "eip155:8453": { gas: "all", bridging: "all", swap: "all" }, // Base: fully sponsored
        "eip155:10": { gas: "all" },                                  // Optimism: only gas
        "eip155:42161": { gas: "deployed" },                           // Arbitrum: gas for deployed accounts only
...
**Sponsored fees** are covered by your sponsorship balance. The user receives the full bridged amount.

**Unsponsored fees** are deducted from the deposit. The user receives less than they sent.

| Sponsorship | User deposits 100 USDC | Fees: 0.50 USDC | User receives |
|---|---|---|---|
| Fully sponsored | 100 USDC | Covered by you | 100 USDC |
| Not sponsored | 100 USDC | Deducted from deposit | 99.50 USDC |
