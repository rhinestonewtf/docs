...
const { userOperation, hash } = data
const sessionSignature = await sessionOwnerAccount.signMessage({
  message: { raw: hash },
})

// Encode and submit
userOperation.signature = encodeSmartSessionSignature(sessionDetails, sessionSignature)
await rhinestoneAccount.submitUserOperation({
  transaction,
  userOperation,
  hash,
