...
To make your first cross-chain transaction, make sure that the account has at least one of the [supported assets](../../home/introduction/supported-chains) on the source chain.
...
You can also send single-chain transactions using the same flow by using the `chain` option:
