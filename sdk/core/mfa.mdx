...
MFA works as a layer on top of other validator modules. It works as a multiplexer for the validator modules, calling the underlying validator implementations to validate the transaction. It also lets you to have multiple configurations for the same validator module.

For example, you can set up an MFA validator so that the user need to sign a transaction with both an EOA and a passkey.
...
Subvalidator ID is the index of the validator in the . For example, if you installed the ECDSA and the passkey modules as subvalidators, the ECDSA validator will have index 0 and the passkey validator will have index 1.
...
You can also install the MFA module to an existing account:
