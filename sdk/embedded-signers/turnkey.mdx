...
```bash
npm install @turnkey/http @turnkey/api-key-stamper @rhinestone/sdk viem
```
...
Create a viem Account instance that integrates with Turnkey for signing:
...
Create a new Rhinestone account using the Turnkey signer:
...
- See our guide on [creating an account](../core/create-account) for more account setup options
- Learn about [sending cross-chain transactions](../core/send-omnichain-transaction) for more transaction details
- Explore [chain abstraction](../chain-abstraction/omnichain-swaps) features for advanced use cases
