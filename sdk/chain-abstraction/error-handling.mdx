...
* `FactoryArgsNotAvailableError`: factory data is not available for this type of an account. Most likely an issue with the SDK itself. Please [submit](https://github.com/rhinestonewtf/sdk/issues) a bug report.
...
* `IntentFailedError`: the intent failed (reverted onchain). Common reason is a swap failed due to price action.
...
* `UnsupportedChainError`: the target or the source chain in not supported. See [Unsupported chain id](#unsupported-chain-id).
...
* `InvalidApiKeyError`: the API key you're using is expired or invalid. See [Invalid API key](#invalid-api-key).
...
* `NoPathFoundError`: we couldn't find a path for the intent you've provided. Try using a different token requests on the target chain.
...
Also make sure you have enough funds for your transaction, taking gas costs into account. You can use the [spendable amount utility](./unified-balance#max-spendable-amount) to get an estimate of how much you can spend.
