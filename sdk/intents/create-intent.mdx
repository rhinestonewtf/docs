...
Intents are declarative transactions that describe **what you want to achieve** rather than **how to achieve it**. With Rhinestone, you can create intents that automatically handle cross-chain complexity, token bridging, and execution optimization.

<Note>The intent system uses the core `sendTransaction` API. Some advanced features shown in examples (like conditional execution, partial fills) represent the intent framework's capabilities and may require additional implementation.</Note>

**What makes intents powerful:**
- Declare your end goal, not the steps
- Automatic route optimization across chains
- Built-in slippage protection and MEV resistance
- Gas-efficient execution with solver competition

## Intent Fundamentals

### Simple Intent Structure

Every intent has three core components:

```tsx
const intent = {
  sourceChains: [arbitrum, base], // Where to find tokens
  targetChain: ethereum,          // Where to execute
  calls: [...],                   // What to execute
  tokenRequests: [...]           // What tokens you need
}
```
