...
Injected Executions are a powerful feature in Rhinestone's intent system that allows the orchestrator to inject additional operations during the fulfillment process. These executions happen at specific points in the cross-chain transaction flow and enable sophisticated operations like token wrapping, protocol interactions, and account setup.

**Key concepts:**
- **Pre-claim operations**: Execute before tokens are claimed on destination chains
- **Destination operations**: Execute as part of the main intent on the target chain
- **Automatic injection**: The orchestrator automatically determines when injections are needed
- **Protocol integration**: Seamless interaction with DeFi protocols and account management
...
```
1. Intent Submission → 2. Route Planning → 3. Pre-claim Executions → 4. Token Bridging → 5. Destination Executions
```

The orchestrator analyzes your intent and determines where additional operations are needed:

### Pre-Claim Executions

Operations that happen before tokens are claimed on destination chains:
...
Operations that happen on the target chain as part of your intent:
