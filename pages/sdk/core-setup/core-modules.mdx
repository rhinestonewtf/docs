...
[**Ownable Validator**](ecdsa-signer.mdx) enables an EOA as a signer for a smart account. It is ideal for product use cases where users are expected to be crypto-native and possess an EOA wallet. Alternatively, it can be combined with MPC providers embedded signers.

[**Webauthn Validator**](passkeys.mdx) enables a passkey as a signer on a smart account, allowing users to sign cryptographic messages with their biometrics via the secure enclave of their device or use a preferred password manager.
