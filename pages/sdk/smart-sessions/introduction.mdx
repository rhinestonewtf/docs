...
Policies let you restrict the session to hit specific conditions. You can define policies at the _session_ (affect the entire session) or _action_ (affects a single action with session) level.

Supported policies include:

- [Sudo](./policies/sudo): allows any action
- Universal action: allows actions with the specified calldata
- Spending limit: allows a limited value of ERC20 tokens transferred and approved
- Timeframe: allows actions within the specified time frame
- Usage limit: allows a limited number of actions
- Value limit: allows a limited ether value transfered with actions
...
You can also limit the session down to specific allowed actions:
...
This will prompt the signature request from the session owner(s), and submit the transaction of their behalf.
