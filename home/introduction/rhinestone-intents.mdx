...
```mermaid
flowchart LR
    User["User\n(smart account or EOA)"]
    SDK["Rhinestone SDK / API"]
    Orch["Orchestrator\n(offchain)"]
    RM["Relayer Market\n(solvers)"]
    Dest["Destination chain\n(fill)"]
    Origin["Origin chain(s)\n(claim)"]

    User -->|"Signs intent"| SDK
    SDK -->|"Submits intent"| Orch
    Orch -->|"Routes + encodes"| RM
    RM -->|"Fills tokens + calls"| Dest
    RM -->|"Claims funds"| Origin
```
...
```mermaid
flowchart TD
    Orch["Orchestrator\nRoute + encode intent operations"]
    IR["Intent Router\nOnchain dispatch to settlement adapters"]
    RM["Relayer Market\nSolvers compete to execute"]

    Across["Across adapter"]
    Relay["Relay adapter"]
    Eco["Eco adapter"]
