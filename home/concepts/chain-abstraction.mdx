...
All smart accounts require transaction infrastructure, even smart EOAs (via EIP-7702). This is the reason ERC-4337 exists. Warp utilizes relayers to support both same-chain and crosschain transactions. This is a huge DevX improvement! One toolkit for all your smart wallet needs.

Read about Intents vs ERC-4337.

### Wide Account Support

Warp is the only chain abstraction system that does not lock developers into a specific account implementation. It is compatible with all smart account implementations and EOAs. This not only provides more freedom to developers but also allows applications to support a wider array of users with an existing smart wallet.

Check out our supported smart accounts.
...
Visit the [Rhinestone Intents](./rhinestone-intent-architecture) for a more thorough intent flow walkthrough.
