    ...
    <table className="min-w-full">
      <thead>
        <tr className="border-b border-gray-200 dark:border-gray-700">
          ...
          <th className="text-left py-2 pr-4 font-semibold">Tokens</th>
          <th className="text-left py-2 font-semibold">Swaps</th>
        </tr>
      </thead>
      <tbody>
        {chains.map((chain) => (
          <tr key={chain.chainId} className="border-b border-gray-100 dark:border-gray-800">
            ...
            <td className="py-2 pr-4">{chain.supportedTokens?.map(t => t.symbol).join(', ') || '-'}</td>
            <td className="py-2">{chain.supportsSwaps ? 'Yes' : 'No'}</td>
          </tr>
        ))}
      </tbody>
    </table>
