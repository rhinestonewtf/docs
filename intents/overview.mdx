...
**Already have smart account users but not using the Rhinestone SDK?** You can still use Warp via the API. You'll need to [install the Intent Executor](/intents/guides/installing-intent-executor) on each account first, then follow the API flow with smart account signing. See the [signing guide](/intents/guides/signing) for the smart account path.
